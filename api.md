...
| POST | `/occurrences/:templateId/:date/files` | `multipart/form-data`, поле `file` | Прикрепить файл к дз (для своей подгруппы). Лимит на один файл — 15 МБ (`400`), плюс общий лимит на все файлы сразу — см. ниже. |

## `/api/homework-files/:id` — файлы дз

Файлы хранятся как `bytea` прямо в Postgres (без отдельного volume) — таблица `homework_files`.

**Общий лимит хранилища — 2 ГиБ.** Перед каждой загрузкой сервер считает `SUM(size_bytes)` по всем файлам во всех предметах у всех пользователей; если новый файл превысит эту сумму — `507 Insufficient Storage`, ничего не сохраняется. Лимит общий для всего приложения (не на пользователя и не на предмет). Единственный способ снова загружать файлы — удалить что-то существующее.
